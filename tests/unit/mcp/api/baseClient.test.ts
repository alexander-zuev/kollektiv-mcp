...
describe("Good ol unit tests of baseClient", () => {
	...
	describe("GET Requests (testing request and getProcessedResponse indirectly)", () => {
		...
		it("should throw ApiError with status 0 for network errors", async () => {
			// Arrange
			const networkError = new Error("Failed to fetch"); // Simulate network issue
			mockFetch.mockRejectedValue(networkError);

			// Act & Assert
			await expect(apiClient.get("/network-error" as any)).rejects.toThrow(ApiError);

			await expect(apiClient.get("/network-error" as any)).rejects.toEqual(
				expect.objectContaining({
					name: "ApiError",
					status: 0,
					statusText: "Network Error",
					message: expect.stringContaining("Network or request setup error: Failed to fetch"),
				}),
			);
	...
	describe("Error Handling", () => {
		...
		it("should throw ApiError for JSON parsing error on success response (200 OK)", async () => {
			...
			await expectApiError(apiCall, 200, "JSON Parsing Error", /Failed to parse JSON response/i);
			expect(mockResponse.json).toHaveBeenCalledTimes(1);
		});
		...
		it("should throw ApiError for network errors (fetch rejects)", async () => {
			...
			await expectApiError(
				...
				0,
				"Network Error",
				/Network or request setup error: Failed to fetch/i,
			);
		});
	});
